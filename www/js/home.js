...
function deleteTagDisplay(sortList,code,callback){
    $.mobile.db.transaction(function(tx) {
...
function changeOrder(responseData){
	$.mobile.db.transaction(function(tx) {
		if(responseData.response != false){
   	        var obj = responseData.response;
   	        for(var key in obj) {
...
function fetchHomeAppData(responseData){
    ...
    bannerData();    
    if(responseData.sorted_tags.length>0){
       	deleteTagDisplay(responseData.sorted_tags, window.localStorage.getItem("selectedCountry"),function(){
