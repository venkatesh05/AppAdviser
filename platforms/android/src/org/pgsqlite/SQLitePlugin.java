...
public class SQLitePlugin extends CordovaPlugin
{
	...
	private void openDatabase(String dbname, String password) 
	{
		...
		if (!dbfile.exists()) {
			File dataBasesLocation = new File(dbfile.getAbsolutePath().replace(dbname+".db", ""));
			if(!dataBasesLocation.exists()){				
				Log.v("Open database ", ""+dataBasesLocation.mkdir());
			}
			Log.v("Open database 1", ""+dbfile);
			InputStream in = this.cordova.getActivity().getAssets().open(dbname + ".db");
	        OutputStream out = new FileOutputStream(dbfile);
	        Log.v("Open database 2", ""+dbfile);
